...
failure_probability = 0.00  # 0% chance of failure per round
...
# Step 4: Select the top 50% of clients based on the combined score
...
# FedAvg Algorithm to average the model weights modified for low learning rate layers
def fed_avg_with_frozen_layers(global_model, client_updates, low_lr_layers):
    """
    Aggregates all the model parameters from client updates and updates the global model.
    """
...
def apply_layer_freezing_with_hysteresis_and_cooldown(
    stability_dict, low_lr_layers, cooldown_counters, 
    freeze_threshold=0.15, 
    unfreeze_threshold=0.25, 
    cooldown=5
):
    """
    Reduces the learning rate of layers based on stability thresholds with hysteresis and cooldown to prevent oscillations.
    
    Args:
        stability_dict (dict): Dictionary containing stability metrics for each layer.
        low_lr_layers (set): Set of currently low learning rate layer names.
        cooldown_counters (dict): Dictionary tracking cooldown for each layer.
        freeze_threshold (float): Threshold below which layers have reduced learning rates.
        unfreeze_threshold (float): Threshold above which layers revert to standard learning rates.
        cooldown (int): Number of rounds to wait before changing layer state.
        
    Returns:
        set: Updated set of low learning rate layer names.
    ...
    for name, metrics in stability_dict.items():
        ...
        if stability < freeze_threshold and name not in low_lr_layers and cooldown_counters[name] >= cooldown:
            low_lr_layers.add(name)
            cooldown_counters[name] = 0  # Reset cooldown
            logging.info(f"Layer {name} has reduced learning rate (stability: {stability:.4f}).")
        
        elif stability > unfreeze_threshold and name in low_lr_layers and cooldown_counters[name] >= cooldown:
            low_lr_layers.remove(name)
            cooldown_counters[name] = 0  # Reset cooldown
            logging.info(f"Layer {name} has reverted to standard learning rate (stability: {stability:.4f}).")
        
        # If not meeting conditions, do not change state and cooldown continues
    return low_lr_layers, cooldown_counters

def log_freezing_status(low_lr_layers, stability_dict):
    """
    Logs the current learning rate status of all layers.
    
    Args:
        low_lr_layers (set): Set of currently low learning rate layer names.
        stability_dict (dict): Dictionary containing stability metrics for each layer.
    """
    logging.info("Current Learning Rate Status:")
    for name, metrics in stability_dict.items():
        status = "Low LR" if name in low_lr_layers else "Standard LR"
        stability = metrics.get('stability', 1.0)
        logging.info(f"Layer {name}: {status}, Stability: {stability:.4f}")

def get_client_update(local_model, low_lr_layers):
    """
    Since all layers are being updated (with some having reduced learning rates),
    send all parameters.
    """
    return {name: param.clone().detach() for name, param in local_model.state_dict().items()}

def get_transmitted_data_size(client_update, low_lr_layers):
    """
    Calculates the size of the transmitted data based on the client update dictionary.
    All parameters are transmitted regardless of low_lr_layers.
    
    Args:
        client_update (dict): A dictionary of parameter tensors from the client.
        low_lr_layers (set): A set of layer names that have reduced learning rates.
    ...
    transmitted_size = sum(
        param.numel() * param.element_size() for param in client_update.values()
    )
    return transmitted_size

def train_client_with_reduced_lr(
    client_loader, model, criterion, device, low_lr_layers, 
    standard_lr=0.01, low_lr_factor=0.1
):
    """
    Trains the client's local model, reducing the learning rate for specified layers.
    
    Args:
        client_loader (DataLoader): DataLoader for the client's local data.
        model (nn.Module): The client's local model.
        criterion (Loss): Loss function.
        device (torch.device): Device to train on.
        low_lr_layers (set): Set of layer names to reduce learning rates.
        standard_lr (float): Standard learning rate.
        low_lr_factor (float): Factor to reduce learning rate by for low_lr_layers.
        
    Returns:
        float: Average loss over the local training.
    ...
    optimizer = optim.Adam([
        {'params': active_params, 'lr': standard_lr},
        {'params': low_lr_params, 'lr': standard_lr * low_lr_factor}
    ])
...
for round in range(num_rounds):
    ...
    # Step 1: Train selected clients
    for client_id in selected_clients:
        local_model = CIFAR10_ResNet().to(device)
        local_model.load_state_dict(global_model.state_dict())
        
        # Train client with reduced learning rates
        client_loss, client_accuracy = train_client_with_reduced_lr(
            client_loaders[client_id], local_model, criterion, device, low_lr_layers, 
            standard_lr=learning_rate, low_lr_factor=0.1
        )
        round_loss.append(client_loss)
        round_accuracy.append(client_accuracy)

        # Extract updates for all layers
        client_update = get_client_update(local_model, low_lr_layers)
        ...
        print(f"Client {client_id + 1}: Transmitted {client_update_size / (1024 * 1024):.2f} MB")
    ...
    if client_updates:
        global_model = fed_avg_with_frozen_layers(global_model, client_updates, low_lr_layers)
    else:
    ...
    # Apply layer freezing with hysteresis and cooldown (now low learning rates)
    low_lr_layers, cooldown_counters = apply_layer_freezing_with_hysteresis_and_cooldown(
        stability_dict, low_lr_layers, cooldown_counters, 
        freeze_threshold=0.15, 
        unfreeze_threshold=0.25, 
        cooldown=5
    )
    logging.info(f"Round {round+1} - Currently low LR layers: {low_lr_layers}")
    print(f"Round {round+1} - Currently low LR layers: {low_lr_layers}")

    # Log current low_lr_layers status
    log_freezing_status(low_lr_layers, stability_dict)
    ...
    total_round_data = sum(get_transmitted_data_size(client_update, low_lr_layers) for client_update in client_updates)
